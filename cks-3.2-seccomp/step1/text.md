...
3. Compare the syscall restrictions by trying `unshare --user` in both pods. Save the results:
   - `"$HOME"/default-unshare.txt` from seccomp-default
   - `"$HOME"/unconfined-unshare.txt` from seccomp-unconfined
4. Document the three seccomp profile types in `"$HOME"/seccomp-types.txt`.
