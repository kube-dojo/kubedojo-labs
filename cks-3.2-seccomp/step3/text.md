...
1. Create `"$HOME"/seccomp-audit.json` — a seccomp profile that logs all syscalls instead of blocking:
...
4. Create `"$HOME"/seccomp-best-practices.txt` with at least 4 seccomp best practices for CKS.
