...
1. Install **trivy** (container image scanner):
   ...
   ```
2. Verify trivy is installed by running `trivy --version` and save the output to `"$HOME"/trivy-version.txt`.
3. Install **kubesec** by downloading its binary:
   ...
   ```
4. Verify kubesec works by running `kubesec version` and save the output to `"$HOME"/kubesec-version.txt`.
