...
3. Back up the current manifest and save key configuration to `"$HOME"/apiserver-config.txt`
...
# Save configuration
grep -E "^\s+--" /etc/kubernetes/manifests/kube-apiserver.yaml > "$HOME"/apiserver-config.txt
