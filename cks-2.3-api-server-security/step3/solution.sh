...
PLUGINS=$(cat "$HOME"/admission-plugins.txt)
COMPLIANT="compliant"
if ! echo "$PLUGINS" | grep -q "NodeRestriction"; then
  COMPLIANT="non-compliant: missing NodeRestriction"
fi
echo "$COMPLIANT" > "$HOME"/admission-check.txt

echo "Default always-on admission controllers: NamespaceLifecycle, LimitRanger, ServiceAccount, DefaultStorageClass, DefaultTolerationSeconds, MutatingAdmissionWebhook, ValidatingAdmissionWebhook, ResourceQuota, Priority" > "$HOME"/default-admission.txt

cat > "$HOME"/admission-security.txt << 'SECURITY'
