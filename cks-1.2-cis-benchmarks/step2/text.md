...
4. Wait for the API server to restart and verify it is healthy. Save the API server status to `"$HOME"/apiserver-status.txt`.

**WARNING**: Editing static pod manifests restarts the component. Save a backup first.

## Hint

Copy the manifest before editing: `cp /etc/kubernetes/manifests/kube-apiserver.yaml "$HOME"/kube-apiserver-backup.yaml`. After editing, the kubelet will restart the pod.
