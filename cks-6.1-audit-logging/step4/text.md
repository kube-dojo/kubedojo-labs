...
1. Create `$HOME/alert-rules.txt` with at least 5 audit-based alert rules in format:
   ```
   RULE: <name>
   ...
   ```
2. Create a script `$HOME/check-secrets-access.sh` that:
   - Reads the audit log
   - Finds all unique users who accessed secrets
   - Reports the count per user
3. Run it and save output to `$HOME/secrets-access-report.txt`.
