...
Find the etcd endpoint URL (the `--listen-client-urls` flag) from the etcd configuration and save it to `$HOME/etcd-endpoint.txt`.
...
```bash
grep listen-client-urls /etc/kubernetes/manifests/etcd.yaml > "$HOME"/etcd-endpoint.txt
```
