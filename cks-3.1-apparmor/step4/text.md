...
1. Check the system log for AppArmor denials: `dmesg | grep -i apparmor` or `journalctl | grep -i apparmor`. Save to `"$HOME"/apparmor-denials.txt`.
2. Create `"$HOME"/troubleshooting-guide.txt` with steps to troubleshoot AppArmor issues:
...
3. Put the `k8s-deny-write` profile in complain mode and save the status to `"$HOME"/complain-mode.txt`.
