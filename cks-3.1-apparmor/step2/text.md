...
3. Test that writing to `/tmp/test` fails inside the pod. Save the result to `"$HOME"/write-test.txt`.
4. Verify the pod is using the AppArmor profile. Save the pod's security context or annotation to `"$HOME"/pod-apparmor.txt`.
