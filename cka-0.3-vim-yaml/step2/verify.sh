...
"
if [ $? -eq 0 ]; then
  echo "PASS: /root/broken.yaml verified with all required fields"
  exit 0
else
  echo "FAIL: /root/broken.yaml is invalid or missing required fields (labels, container port, etc.)"
  exit 1
fi