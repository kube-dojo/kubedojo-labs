...
1. Apply the staging overlay: `kubectl apply -k "$HOME"/kustomize/overlays/staging/`
2. Apply the production overlay: `kubectl apply -k "$HOME"/kustomize/overlays/production/`
3. Verify staging has 2 replicas and production has 5 replicas.
4. Save the staging deployment name to `"$HOME"/staging-deploy-name.txt`:
   ```
