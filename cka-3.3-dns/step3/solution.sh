...
kubectl exec dnsutils -n frontend -- nslookup kubernetes.default > "$HOME"/dns-debug.txt 2>&1
COREDNS_POD=$(kubectl get pods -n kube-system -l k8s-app=kube-dns -o jsonpath='{.items[0].metadata.name}')
kubectl logs $COREDNS_POD -n kube-system --tail=20 > "$HOME"/coredns-logs.txt 2>&1
