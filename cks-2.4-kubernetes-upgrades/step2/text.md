...
2. Save the target version to `"$HOME"/target-version.txt`.
3. Create `"$HOME"/upgrade-commands.txt` documenting the exact commands you would run for a real upgrade:
   ...
   - Restart kubelet
   - Uncordon the node
4. Save the current etcd version to `"$HOME"/etcd-version.txt` (from the etcd pod or manifest).
