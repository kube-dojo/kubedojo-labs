...
1. List current iptables rules and save to `"$HOME"/iptables-before.txt`.
2. Create iptables rules that:
   ...
   - Save the rules to `"$HOME"/iptables-rules.txt` (the commands, not applied — do not disrupt the cluster)
3. Create `"$HOME"/firewall-best-practices.txt` with at least 4 node firewall best practices.
