...
2. Verify the pod starts successfully. Save its status to `"$HOME"/restricted-ok.txt`.
3. Try creating a **non-compliant** pod named `restricted-fail` in `psa-restricted` with just `image: nginx` (no security context). Save the error to `"$HOME"/restricted-fail.txt`.
4. Document the requirements of the restricted standard in `"$HOME"/restricted-requirements.txt`.
