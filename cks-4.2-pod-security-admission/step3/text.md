...
2. Deploy a pod with `image: nginx` (no security context) in `psa-transition`. It should succeed (passes baseline) but show warnings. Save the output including warnings to `"$HOME"/psa-warnings.txt`.
3. Create `"$HOME"/psa-migration-plan.txt` documenting a plan to migrate from baseline to restricted:
