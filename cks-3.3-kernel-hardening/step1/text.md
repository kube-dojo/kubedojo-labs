...
1. Create a pod named `sysctl-pod` in `kernel-lab` with safe sysctl settings:
   ...
   securityContext:
     ...
     - name: net.ipv4.ping_group_range
       value: "0 65535"
   ```
2. Save the list of safe sysctls (allowed by default) to `"$HOME"/safe-sysctls.txt`.
3. Save the list of common unsafe sysctls that are relevant to security to `"$HOME"/unsafe-sysctls.txt`.
4. Check the current kernel parameters on the node and save security-relevant ones to `"$HOME"/node-sysctls.txt`:
   - `net.ipv4.ip_forward`
   - `kernel.panic`
