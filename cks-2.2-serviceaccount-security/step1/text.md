...
3. Verify the pod does not have a mounted token by checking if `/var/run/secrets/kubernetes.io/serviceaccount/token` exists inside the container. Save the result to `"$HOME"/token-check.txt`.
