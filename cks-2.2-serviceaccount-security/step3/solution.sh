#!/bin/bash
kubectl get sa -A > "$HOME"/all-service-accounts.txt

echo "" > "$HOME"/sa-permissions.txt
for sa in $(kubectl get sa -n sa-lab -o jsonpath='{.items[*].metadata.name}'); do
  ...
  echo "$sa: list secrets = $RESULT" >> "$HOME"/sa-permissions.txt
done

kubectl get secrets -A --field-selector type=kubernetes.io/service-account-token > "$HOME"/legacy-tokens.txt 2>&1 || echo "No legacy tokens found" > "$HOME"/legacy-tokens.txt

cat > "$HOME"/sa-hardening.txt << 'BEST'
