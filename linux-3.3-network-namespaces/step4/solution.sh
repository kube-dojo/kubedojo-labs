...
sudo ip netns exec lab-ns ping -c1 -W2 "$HOST_IP" > "$HOME"/isolation-test.txt 2>&1 || true

# If the output is empty (no route at all), provide a meaningful message
if [ ! -s "$HOME"/isolation-test.txt ]; then
  echo "Isolation confirmed: lab-ns cannot reach host IP $HOST_IP (network unreachable)" > "$HOME"/isolation-test.txt
fi
