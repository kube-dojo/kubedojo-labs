...
#!/bin/bash
# Verify $USER_HOME/root-fstype.txt contains a known filesystem type
if [ ! -f $USER_HOME/root-fstype.txt ]; then
  echo "$USER_HOME/root-fstype.txt does not exist"
  exit 1
fi

fstype=$(cat $USER_HOME/root-fstype.txt | tr -d '[:space:]')
