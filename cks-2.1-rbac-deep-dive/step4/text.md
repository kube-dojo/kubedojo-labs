...
2. Verify that a ServiceAccount with this role could potentially escalate privileges by creating new roles. Save the `can-i` check to `"$HOME"/escalation-check.txt`.
3. Create a more secure version named `safe-role-creator` that:
   - Allows only `get`, `list` on `roles` and `rolebindings` (no create)
4. Document in `"$HOME"/escalation-prevention.txt` three strategies to prevent RBAC escalation.
