...
kubectl auth can-i create roles -n rbac-lab --as=system:serviceaccount:rbac-lab:secret-admin > "$HOME"/escalation-check.txt 2>&1

cat > "$HOME"/escalation-prevention.txt << 'PREVENT'
