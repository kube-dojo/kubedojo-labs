## Resource Management — Quotas, Limits & Priorities

Welcome to this hands-on lab from [KubeDojo](https://kube-dojo.github.io/) — free, open-source cloud native education.

You have a real Kubernetes cluster with one control plane node ready to use.

### Objectives

In this lab you will:

- Create ResourceQuotas to limit namespace consumption
- Configure LimitRanges for default container resources
- Define PriorityClasses for scheduling priority
- Test quota enforcement by exceeding limits

### Before You Start

- Read the [module page](https://kube-dojo.github.io/k8s/cka/part2-workloads-scheduling/module-2.5-resource-management/) for theory
- This lab takes approximately **40 minutes**
- `kubectl` is already configured and ready
- Alias `k=kubectl` is set up for speed

Let's begin!
