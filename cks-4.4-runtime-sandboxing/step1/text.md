...
1. Check the current container runtime on the node: `kubectl get nodes -o jsonpath='{.items[0].status.nodeInfo.containerRuntimeVersion}'`. Save to `"$HOME"/current-runtime.txt`.
2. List any existing RuntimeClass resources: `kubectl get runtimeclass`. Save to `"$HOME"/existing-runtimeclasses.txt`.
3. Create `"$HOME"/runtime-comparison.txt` comparing three runtimes:
