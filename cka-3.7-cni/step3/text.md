...
3. Save the pod's IP address to `"$HOME"/pod-ip.txt`
...
# Save the IP
kubectl get pod network-test -n practice -o jsonpath='{.status.podIP}' > "$HOME"/pod-ip.txt
