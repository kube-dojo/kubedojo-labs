...
1. Create a script `"$HOME"/cluster-scan.sh` that:
   - Lists all unique container images running in the cluster
   - Scans each image with trivy (CRITICAL and HIGH only)
   - Outputs a summary report
2. Run the script and save output to `"$HOME"/cluster-scan-results.txt`.
3. Create `"$HOME"/scanning-best-practices.txt` with at least 4 scanning best practices.
