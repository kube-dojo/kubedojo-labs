...
1. Count CVEs by severity for `nginx:1.19` and save to `"$HOME"/cve-counts.txt`:
   - CRITICAL: X
   - HIGH: X
   - MEDIUM: X
   - LOW: X
2. Filter for only fixable vulnerabilities: `trivy image --ignore-unfixed nginx:1.19` and save to `"$HOME"/fixable-cves.txt`.
3. Create `"$HOME"/cve-priority.txt` documenting your prioritization strategy:
