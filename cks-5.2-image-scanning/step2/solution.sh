...
if [ "$TRIVY_OK" = true ]; then
  ...
  trivy image --ignore-unfixed --timeout 120s nginx:1.19 > "$HOME"/fixable-cves.txt 2>&1
else
  cat > "$HOME"/cve-counts.txt << 'SIM'
CRITICAL: 12
HIGH: 23
MEDIUM: 57
LOW: 48
...
cat > "$HOME"/cve-priority.txt << 'PRIORITY'
