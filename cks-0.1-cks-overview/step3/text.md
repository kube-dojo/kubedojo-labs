...
2. Identify which pod is running as privileged and save its name to `"$HOME"/insecure-pod.txt`.
3. Identify which pod has `readOnlyRootFilesystem: true` and save its name to `"$HOME"/secure-pod.txt`.
4. Create a file `"$HOME"/security-fixes.txt` listing three security improvements for the insecure pod (one per line).
