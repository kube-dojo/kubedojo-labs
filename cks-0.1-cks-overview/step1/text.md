...
1. Check the Kubernetes version and save it to `"$HOME"/k8s-version.txt`.
...
3. List all namespaces and save them to `"$HOME"/namespaces.txt`.
4. Examine the API server manifest at `/etc/kubernetes/manifests/kube-apiserver.yaml` and save any `--enable-admission-plugins` value to `"$HOME"/admission-plugins.txt`.
