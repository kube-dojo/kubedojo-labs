...
if [ -f "$MANIFEST" ]; then
  grep 'enable-admission-plugins' "$MANIFEST" > "$HOME"/enabled-plugins.txt || echo "Using defaults" > "$HOME"/enabled-plugins.txt
else
  NODE=$(docker ps --filter "name=control-plane" --format "{{.Names}}" 2>/dev/null | head -1)
  if [ -n "$NODE" ]; then
    docker exec "$NODE" grep 'enable-admission-plugins' /etc/kubernetes/manifests/kube-apiserver.yaml > "$HOME"/enabled-plugins.txt 2>/dev/null || echo "Using defaults" > "$HOME"/enabled-plugins.txt
  else
    echo "Using defaults: NodeRestriction" > "$HOME"/enabled-plugins.txt
  fi
fi

cat > "$HOME"/security-plugins.txt << 'PLUGINS'
...
if [ -f "$MANIFEST" ]; then
  if grep -q "AlwaysPullImages" "$MANIFEST"; then
    echo "enabled" > "$HOME"/always-pull-status.txt
  else
    echo "disabled" > "$HOME"/always-pull-status.txt
  fi
else
  NODE=$(docker ps --filter "name=control-plane" --format "{{.Names}}" 2>/dev/null | head -1)
  if [ -n "$NODE" ]; then
    docker exec "$NODE" grep -q "AlwaysPullImages" /etc/kubernetes/manifests/kube-apiserver.yaml 2>/dev/null && echo "enabled" > "$HOME"/always-pull-status.txt || echo "disabled" > "$HOME"/always-pull-status.txt
  else
    echo "disabled" > "$HOME"/always-pull-status.txt
  fi
fi

cat > "$HOME"/admission-flow.txt << 'FLOW'
API Request Flow:
