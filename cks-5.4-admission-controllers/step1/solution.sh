#!/bin/bash
grep 'enable-admission-plugins' /etc/kubernetes/manifests/kube-apiserver.yaml > /root/enabled-plugins.txt || echo "Using defaults" > /root/enabled-plugins.txt
...
if grep -q "AlwaysPullImages" /etc/kubernetes/manifests/kube-apiserver.yaml; then
