...
1. Create a ConfigMap named `app-config` in `cm-lab` from literals:
   ...
   ```
2. Create a ConfigMap named `file-config` in `cm-lab` from the file `$HOME/app.properties`:
   ```
   kubectl create configmap file-config --from-file=$HOME/app.properties -n cm-lab
   ```
3. Verify both ConfigMaps exist and save the data of `app-config` to `$HOME/cm-data.txt`:
   ```
