#!/bin/bash
kubectl exec test-pod -n metadata-lab -- curl -s --connect-timeout 3 http://169.254.169.254/ > "$HOME"/metadata-test-after.txt 2>&1 || echo "Connection blocked by NetworkPolicy" > "$HOME"/metadata-test-after.txt
cat > "$HOME"/metadata-controls.txt << 'CONTROLS'
