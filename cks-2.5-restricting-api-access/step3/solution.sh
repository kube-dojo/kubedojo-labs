...
if [ -f "$MANIFEST" ]; then
  grep -E '(max-requests|max-mutating)' "$MANIFEST" > "$HOME"/rate-limits.txt || echo "Using defaults: max-requests-inflight=400, max-mutating-requests-inflight=200" > "$HOME"/rate-limits.txt
else
  NODE=$(docker ps --filter "name=control-plane" --format "{{.Names}}" 2>/dev/null | head -1)
  if [ -n "$NODE" ]; then
    docker exec "$NODE" grep -E '(max-requests|max-mutating)' /etc/kubernetes/manifests/kube-apiserver.yaml > "$HOME"/rate-limits.txt 2>/dev/null || echo "Using defaults: max-requests-inflight=400, max-mutating-requests-inflight=200" > "$HOME"/rate-limits.txt
  else
    echo "Using defaults: max-requests-inflight=400, max-mutating-requests-inflight=200" > "$HOME"/rate-limits.txt
  fi
fi

cat > "$HOME"/rate-limit-docs.txt << 'DOCS'
...
cat > "$HOME"/event-rate-limit.txt << 'ERL'
...
cat > "$HOME"/api-hardening-summary.txt << 'SUMMARY'
