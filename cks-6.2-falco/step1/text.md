...
2. Check if Falco is running: `systemctl status falco` and save to `$HOME/falco-status.txt`.
3. List the default Falco rules file locations and save to `$HOME/falco-rules-paths.txt`.
4. Save a sample of the default rules to `$HOME/default-rules-sample.txt` (first 50 lines of `/etc/falco/falco_rules.yaml`).
