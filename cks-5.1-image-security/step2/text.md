...
1. Create an OPA/Gatekeeper-style constraint template concept document at `"$HOME"/image-policy.yaml` showing how you would:
   - Allow only images from `docker.io/library/` and `registry.k8s.io/`
   - Deny all other registries
2. Without an admission webhook, enforce a simpler check: create a script at `"$HOME"/check-images.sh` that:
...
3. Run the script and save output to `"$HOME"/image-compliance.txt`.
4. Create `"$HOME"/approved-registries.txt` listing registries you would allow in production.
